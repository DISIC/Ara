...
export class ImageImportPlugin extends Plugin {
  ...
  private handleImageImport(
    ...
    if (!file.type.startsWith("image")) {
      this.notify("error", undefined, FileMessage.UPLOAD_ERROR_FORMAT);
      return;
    }
  ...
  private uploadAndReplacePlaceholder(
    ...
    this.uploadImage(file).then(
      ...
      async (reason: Error) => {
        ...
        const desc = await this.fileHandler.handleFileUploadError(reason, file.name);
        this.notify("error", undefined, desc ?? undefined);
      }
    );
  }
